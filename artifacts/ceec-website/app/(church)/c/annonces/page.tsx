...
async function getAnnonces(egliseId: number) {
  try {
...
export default async function ChurchAnnoncesPage() {
  ...
  const annoncesList = await getAnnonces(eglise.id);
  ...
  return (
    <>
      <section style={{
        ...
        <h1 style={{ fontSize: "2.25rem", fontWeight: 800, marginBottom: 10 }}>Annonces</h1>
        <p style={{ opacity: 0.8, fontSize: 15 }}>
          Dernières nouvelles de {eglise.nom}
        </p>
      </section>
          ...
          {annoncesList.length === 0 ? (
            <div style={{
              ...
              <p style={{ color: "#64748b" }}>
                Les annonces de {eglise.nom} seront publiées ici.
              </p>
            </div>
          ) : (
                ...
                return (
                  <div key={annonce.id} style={{
                    ...
                    <div style={{
                      ...
                      marginBottom: 12, flexWrap: "wrap" as const, gap: 8,
                    }}>
                      <span style={{
                    ...
                    <h2 style={{ fontWeight: 700, color: "#0f172a", marginBottom: 10, fontSize: 18, margin: "0 0 10px" }}>
                      {annonce.titre}
                    </h2>
                    <p style={{ color: "#475569", lineHeight: 1.8, margin: 0 }}>{annonce.contenu}</p>
                    {annonce.dateExpiration && (
                      <p style={{ color: "#94a3b8", fontSize: 12, marginTop: 12, margin: "12px 0 0" }}>
                        Expire le {new Date(annonce.dateExpiration).toLocaleDateString("fr-FR", {
                          day: "numeric", month: "long", year: "numeric",
                        })}
                      </p>
                    )}
